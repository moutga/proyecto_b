class Api {
	getTodosContactos() {
		return new Promise(function(resolve, reject) {
			(async function() {
				const contactos = await JSON.parse(
	...
	async filtrarContactos(valor) {
		...
		return new Promise(function(resolve, reject) {
			...
			let filtrado = contactos.filter(function(c) {
				//contacto
				let b = [];
				...
				keysCompuestas.forEach(function(k) {
					//emails.email

					let key = k.split(".")[0]; //emails
					let subKey = k.split(".")[1]; //email

					c[key].forEach(function(kk) {
						//contacto.emails
	...
	async guardar(nuevo) {
		...
		return new Promise(function(resolve, reject) {
			...
			if (!email && !telefono) {
				reject("El contacto debe tener un email o un teléfono");
				return;
			}
			if (!email[0].email && !telefono[0].telefono) {
				reject("El contacto debe tener un email o un teléfono");
				return;
			}
	...
	async actualizar(conData) {
		...
		return new Promise(function(resolve, reject) {
			...
			conData.nombre
				? (contactos[idxContacto].nombre = conData.nombre)
				: false;
			conData.domicilio
				? (contactos[idxContacto].domicilio = conData.domicilio)
				: false;
			conData.etiquetas
				? (contactos[idxContacto].etiquetas = conData.etiquetas)
				: false;
			...
			if (conData.telefonos) {
				...
				if (listaTelefonos.length - 1 < 0) {
					newTelefonoId = 1;
				}

				conData.telefonos[0].id = newTelefonoId;

				contactos[idxContacto].telefonos.push(conData.telefonos[0]);
			}
			...
			if (conData.emails) {
				...
				if (listaEmails.length - 1 < 0) {
					newEmailId = 1;
				}

				conData.emails[0].id = newEmailId;

				contactos[idxContacto].emails.push(conData.emails[0]);
			}
