...
class Auth {
	...
	guardar(usuData) {
		...
		let rolCorrecto =
			usuData.rol == "USUARIO" || usuData.rol == "ADMINISTRADOR"
				? true
				: false;
		let todosDatos =
			usuData.nombre &&
	...
	actualizar(usuData) {
		...
		return new Promise(function (resolve, reject) {
			...
			if (!hayId) {
				reject("Hace falta ID de usuario");
				return;
			}
			if (!rolCorrecto) {
				reject("Rol incorrecto");
			...
			if (usuarios) {
				...
				if (!idValido) {
					reject("La ID no corresponde a un usuario");
					return;
				}
				...
				usuData.usuario
					? (usuarios[indexUsuario].usuario = usuData.usuario)
					: false;
				usuData.rol
					? (usuarios[indexUsuario].rol = usuData.rol)
					: false;
				usuData.nombre
					? (usuarios[indexUsuario].nombre = usuData.nombre)
					: false;
				usuData.contrasena
					? (usuarios[indexUsuario].contrasena = md5(
						usuData.contrasena
					))
					: false;
