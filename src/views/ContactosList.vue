...
<div>
	...
	<v-container class="mb-3">
		...
		<transition-group name="list">
			...
			<ContactoItem :ref="'item'+contacto.id" @enviaContactoId="abreDialogo" :key="contacto.id" :nombre="contacto.nombre" :id="contacto.id" v-for="(contacto,i) in contactos" class="list-item" />
	...
	<v-container>
	<v-row >
		<router-link class="ml-auto" :to="{ name: 'contacto', params: { id: -1 }}">
		<v-btn color="green" elevation="1" outlined>
			<v-icon left>mdi-account-plus-outline</v-icon> Añadir contacto
		</v-btn>
		</router-link>
...
<script>
// import UsuarioItem from "@/components/ContactoItem.vue";
...
export default {
	...
	data: function() {
		return {
			cargandoLista: false,
			contactos: null, //* La lista de contactos
			dialog: false,
	...
	methods: {
		...
		aceptaBorrar: function(){
			...
			//* y lo quito del array contactos
			let cualBorroRender = this.contactos.findIndex(function(u){
				return u.id == borrando
			});
			this.contactos.splice(cualBorroRender,1);
			Api.borrar(borrando);
	...
	mounted: async function() {
		...
		this.contactos = await Api.getTodosContactos();
